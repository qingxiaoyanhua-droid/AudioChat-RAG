...
matcha_tts_path = os.path.join(
    current_dir, "../", "third_party/CosyVoice/third_party/Matcha-TTS"
)
...
def token2wav(
    cosyvoice3_model, tokens, embedding=None, token_hop_len=25 * 30, pre_lookahead_len=3
):
    ...
    # Step 1: Split audio into 30-second segments （分块 每 25 tokens 对应约 1 秒音频， 30s 一个token）
...
def get_audio_detokenizer(token_hop_len=25 * 30):
    ...
    cosyvoice3 = CosyVoice3(
        "/data/models/Voice/FunAudioLLM/Fun-CosyVoice3-0.5B-2512",
        load_trt=False,
        load_vllm=False,
        fp16=False,
    )
